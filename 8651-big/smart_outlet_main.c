...
//    printf("*********************VSON2\n");
    ...
    *p = 0;
    printf("data:%s\n",buf);
    extern void bump_receive_server(uint8_t * data, int len);
    bump_receive_server(payload, payload_len);
//    printf("ASCII:%s\n",payload);
    return 0;
}
...
static char timeValid_ = 0;
void SetTimeState(int status)
{
    timeValid_ = status;
}
char GetTimeState(void)
{
    return timeValid_;
}
...
static int user_timestamp_reply_event_handler(const char *timestamp)
{
    ...
    for(i = num - 1; i >= 0 ; i--)
    {
        utc += ((uint32_t)(timestamp[i] - '0')) * pow(10 , num - 1 - i);
    }
    
    extern uint32_t CRUENT_TIME;
    CRUENT_TIME = utc / 1000;

    SetTimeState(1);  //获取时间戳成功
...
//    readtest();
    ...
    IOT_RegisterCallback(ITE_TIMESTAMP_REPLY, user_timestamp_reply_event_handler);  //打开，才能读取到时间
    ...
    if ((0 == strlen(master_meta_info.product_key)) || (0 == strlen(master_meta_info.device_name))
        ...
        LOG_TRACE("No device meta info found...2\n");
        while (1) {
